...
export default function Navbar({ onLogout, navigateTo, userInfo }) {
  return (
      ...
      <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-6">
          ...
          {userInfo && userInfo.isAdmin && (
            <button onClick={() => navigateTo('admin-dashboard')} className="text-red-600 font-bold hover:text-red-800">
              Admin Dashboard
            </button>
          )}
        </div>
        <div className="flex items-center space-x-4">
          <span className="hidden sm:inline text-gray-700 font-medium">Welcome, {userInfo.name}!</span>
          <button
            onClick={onLogout}
